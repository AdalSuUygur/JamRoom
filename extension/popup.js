...
document.getElementById('leaveBtn').addEventListener('click', () => {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.storage.local.remove(['savedRoomId', 'roomUserCount', 'roomQueue']);
    setStatus(chrome.i18n.getMessage('statusDefault'), false);
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  state.inRoom = true;
  setStatus(chrome.i18n.getMessage('statusJoined', [msg.roomId]), true);
  updateCopyVisibility(true);
  updateQueueVisibility(true);

  // Wait briefly for first server push (userListUpdate + queueUpdate).
  setTimeout(() => {
    chrome.storage.local.get(['roomUserCount', 'roomUserList', 'roomQueue'], (res) => {
      const count = res.roomUserCount || 1;
      setStatus(chrome.i18n.getMessage('statusInRoom', [String(count)]), true);
...
chrome.storage.onChanged.addListener((changes) => {
  ...
  if (changes.roomUserCount) {
    ...
    setStatus(chrome.i18n.getMessage(key, args), count > 0);
  }
...
chrome.storage.local.get(['savedRoomId', 'roomUserCount', 'roomUserList', 'roomQueue', 'savedNickname'], (result) => {
...
async function addToQueue() {
  ...
  if (!videoId) {
    showToast(chrome.i18n.getMessage('queueInvalidUrl'));
    return;
  }
  ...
  chrome.storage.local.get(['savedRoomId', 'savedNickname'], (res) => {
    if (!res.savedRoomId) {
      showToast(chrome.i18n.getMessage('queueLockedMsg'));
      resetAddBtn();
      return;
    }
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      queueInput.value = '';
      resetAddBtn();
      showToast(chrome.i18n.getMessage('queueAdded'));
    });
  });
}
...
function renderQueue(queue) {
  if (!queue?.length) {
    ...
    queueList.innerHTML   = `<div class="queue-empty">${chrome.i18n.getMessage('queueEmpty')}</div>`;
    return;
  }

  const total = queue.length;
  queueMeta.innerHTML = `<span>${total}</span> ${chrome.i18n.getMessage(total === 1 ? 'queueCountOne' : 'queueCountMany')}`;
